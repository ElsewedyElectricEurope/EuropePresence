...
function partnerCardHTML(p, countryName){
  return `
    <div class="cp-card" data-name="${p.name}">
      ...
      <div class="cp-count"><b>${fmtNum(p.count)}</b> opportunit${p.count===1?'y':'ies'}</div>
...
function renderPartnersPanel(){
  ...
  if(hasCountry){
    ...
    cpHeadEl.innerHTML = `
      <div class="cp-head-left">
        ...
        <div>
          ...
          <div class="cp-subtitle">${hasCategory ? `Showing ${selectedCategory} partners · ` : ``}<b>${list.length} partner${list.length===1?'':'s'}</b>${d ? ` · <b>${fmtNum(d.count)} opportunities won</b>` : ``}</div>
        </div>
      </div>
    ...
    cpGridEl.innerHTML = shown.map(p=>partnerCardHTML(p, COUNTRY_DATA[p.code] ? COUNTRY_DATA[p.code].name : "")).join("")
      + (all.length > shown.length ? `<div class="cp-truncnote" style="grid-column:1/-1;">Showing top ${shown.length} of ${all.length} by opportunity count.</div>` : ``);
  }
...
function showTooltip(a2, evt){
  ...
  tooltip.innerHTML = `
    ...
    <div style="font-size:12px; color:var(--text-1); margin-bottom:8px;">${fmtNum(d.count)} opportunities · ${fmtUSD(d.amount)}</div>
    ${top.map(p=>`
